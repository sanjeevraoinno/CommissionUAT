...
if (true) {
    ...
    if (true) {
        ...
        WebUI.setText(findTestObject('Page_Callidus Cloud Commissions Man/td_Value'), 'F_V_demo')

        WebUI.delay(2)

        WebUI.click(findTestObject('Page_Callidus Cloud Commissions Man/input_Apply Search'))

        println('User search for Fixed Values successfull, Testcase , Pass')
		
    } else {
        false
    }
    
    println('Fixed Valuess screen is unavailable Test case FAIL')
} else {
    false.call({ 
